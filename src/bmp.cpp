...
namespace sm::images
{
    ...
    void BMP::Interpolate()
    {
        for (int i = 0; i < m_height; i++)
        {
            for (int j = 0; j < m_width; j++)
            {
                if (m_pixels[i][j].b != 0 && m_pixels[i][j].g != 0 && m_pixels[i][j].r != 0) continue;
                ...
                for (int k = i - 1; k <= i + 1; k++)
                {
                    for (int l = j - 1; l <= j + 1; l++)
                    {
                        ...
                        Pixel tmp = m_pixels[k][l];
                        if (tmp.b != 0 || tmp.g != 0 || tmp.r != 0)
                        {
                            blue += tmp.b;
                            green += tmp.g;
                            red += tmp.r;
                ...
                if (counter>4)
                {
                    m_pixels[i][j].b = (unsigned char)(blue / counter);
                    m_pixels[i][j].g = (unsigned char)(green / counter);
                    m_pixels[i][j].r = (unsigned char)(red / counter);
                }
            }
        }
